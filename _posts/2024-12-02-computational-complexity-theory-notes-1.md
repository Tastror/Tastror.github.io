...
因此，在处理问题时，我们首先会判断它是否可以识别、计算（可识别性与可判定性），在可以计算的基础上，会关心计算它至少要花费多少时间与空间（时间复杂性类与空间复杂性类），是否有别的模型可以更进一步进行计算（非确定性图灵机、概率图灵机、交互图灵机等产生的复杂性类），以及，理论上是否存在一个可达的更快的方法（复杂性类的包含于等价关系，例如，$\mathbf{P} \overset{\text{?}}{=} \mathbf{NP}$）．
